...
if [ "$STATUS" = "Playing" ] || [ "$STATUS" = "Paused" ]; then
    ...
    elif [[ "$sink_name" == *"pci"* ]] || [[ "$sink_name" == *"analog"* ]]; then
        active_port=$(pactl --format=json list sinks 2>/dev/null | python3 -c "
import json,sys
for s in json.load(sys.stdin):
    if s.get('name','') == '$sink_name':
        print(s.get('active_port',''));break" 2>/dev/null)
        if [[ "$active_port" == *"headphones"* ]]; then
            dev_icon="󰋎"; dev_name="Headphones"
        else
            dev_icon="󰓃"; dev_name="Line Out"
        fi
    fi
