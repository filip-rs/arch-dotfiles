...
Item {
    ...
    // Port toggle state (headphones vs line out)
    property string currentPort: "lineout"
    property bool portAvailable: false

    Process {
        id: portPoller
        command: ["bash", window.scriptsDir + "/audio_control.sh", "get-port"]
        stdout: StdioCollector {
            onStreamFinished: {
                let result = this.text.trim();
                if (result !== "none") {
                    window.portAvailable = true;
                    window.currentPort = result;
                } else {
                    window.portAvailable = false;
                }
            }
        }
    }
    ...
    Timer {
        ...
        onTriggered: portPoller.running = true
    }
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        ColumnLayout {
                            ...
                            // Port toggle (headphones / line out)
                            Rectangle {
                                visible: window.activeTab === "outputs" && window.portAvailable
                                Layout.preferredHeight: window.s(28)
                                Layout.preferredWidth: portPillRow.width + window.s(20)
                                radius: window.s(14)
                                color: portMa.containsMouse ? "#1affffff" : "#0dffffff"
                                border.color: portMa.containsMouse ? window.tabColor : "#1affffff"
                                border.width: 1
                                ...
                                RowLayout {
                                    ...
                                    spacing: window.s(6)
                                    Text {
                                        text: window.currentPort === "headphones" ? "󰋎" : "󰓃"
                                        color: window.tabColor
                                        font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14)
                                    }
                                    Text {
                                        text: window.currentPort === "headphones" ? "Headphones" : "Line Out"
                                        color: window.subtext0
                                        font.family: "JetBrains Mono"; font.bold: true; font.pixelSize: window.s(11)
                                    }
                                }
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        Quickshell.execDetached(["bash", window.scriptsDir + "/audio_control.sh", "toggle-port"]);
                                        portPoller.running = true;
                                    }
                                }
                            }
